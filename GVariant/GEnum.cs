...
namespace GVariant
{
    public class GEnum : GVariant
    {
        ...
        public static GEnum Build(string name, IEnumerable<KeyValuePair<string, int>> members, bool isFlags = false)
        {
            ...
            Type gType = isFlags ? typeof(GFlagsEnum<>) : typeof(GEnum<>);
            Type gGenType = gType.MakeGenericType(new Type[] { type });
            var ctor = gGenType.GetConstructor(new Type[0])!;
    ...
    public class GEnum<T> : GEnum, GVariant<T> where T : Enum
    {
        ...
        public virtual T Deserialize(string encoded)
        {
            string name = GVariantUtils.Unquote(encoded);
            return Enum.TryParse(typeof(T), name, out object? result)
                ? (T)result
    ...
    public class GFlagsEnum<T> : GEnum<T>, GVariant<T> where T : Enum
    {
        ...
        public override T Deserialize(string encoded)
        {
            ...
            {
                if (!Enum.TryParse(typeof(T), n, out _))
                {
                    throw new ParseException($"{n} is not a valid case for {typeof(T)}");
                }
            }
            return (T)Enum.Parse(typeof(T), string.Join(',', names));
        }
    }
}
